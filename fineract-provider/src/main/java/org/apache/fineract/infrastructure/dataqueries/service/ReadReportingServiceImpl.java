...
public class ReadReportingServiceImpl implements ReadReportingService {
    ...
    public GenericResultsetData retrieveGenericResultset(final String name, final String type, final Map<String, String> queryParams,
        ...
        LOG.info("String sql 183: " + sql); // add natha

        final GenericResultsetData result = this.genericDataService.fillGenericResultSet(sql);

        // add natha
        // if (name.equals("Active Loans - Details(Pentaho)")) {
        if ("Active Loans - Details(Pentaho)".equals(name)) {
            String sql2 = "select x.* from ((select lo.id, lo.display_name as `Name` \n" + "from m_office o \n"
                    + "join m_office ounder on ounder.hierarchy like concat(o.hierarchy, '%')\n"
                    + "join m_staff lo on lo.office_id = ounder.id\n" + "where lo.is_loan_officer = true\n" + "and o.id = 1)\n"
                    + "union all\n" + "(select -10, '-')\n" + "order by 2) x";
            LOG.info("String sql2 193: " + sql2); // add natha
            GenericResultsetData result2 = this.genericDataService.fillGenericResultSet(sql);
            // result = result2;
            LOG.info("String sql2 196: " + result2.getData().toString()); // add natha
        }
        // end add
    ...
    public GenericResultsetData retrieveGenericResultSetForSmsEmailCampaign(String name, String type, Map<String, String> queryParams) {
        ...
        LOG.info("String sql 516: " + sql); // natha
